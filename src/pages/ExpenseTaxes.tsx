...
const ENDPOINT = '/api/v1/expenses/taxes/';
const crud = createCrudHooks<Record<string, unknown>>({
  queryKey: 'expense-taxes',
  endpoints: { list: ENDPOINT, create: ENDPOINT, detail: (id) => `${ENDPOINT}${id}/`, update: (id) => `${ENDPOINT}${id}/`, delete: (id) => `${ENDPOINT}${id}/` },
});

const columns: ColumnDef<Record<string, unknown>>[] = [
  { key: 'name', header: 'Name', sortable: true },
  { key: 'tax_type', header: 'Tax Type', sortable: true },
  { key: 'rate_percentage', header: 'Rate %', sortable: true },
  { key: 'is_inclusive', header: 'Inclusive', sortable: true, render: (v: unknown) => (v ? 'Yes' : 'No') },
...
const createFields: FieldConfig[] = [
  { key: 'name', label: 'Name', type: 'text', required: true, placeholder: 'e.g. GST 18%' },
  { key: 'tax_type', label: 'Tax Type', type: 'select', required: true, options: [
    { value: 'gst', label: 'GST' },
    { value: 'vat', label: 'VAT' },
    { value: 'other', label: 'Other' },
  ]},
  { key: 'rate_percentage', label: 'Rate (%)', type: 'number', required: true, placeholder: '18' },
  { key: 'is_inclusive', label: 'Tax Inclusive', type: 'checkbox' },
];

export default function ExpenseTaxes() {
    ...
    ...columns,
    {
      ...
      render: (_value: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-1">
          <EditButton permission="expense_taxes.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
          <DeleteButton permission="expense_taxes.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
        </div>
      ),
    },
  ];
  ...
  const handleEdit = async (data: Record<string, unknown>) => {
    ...
    setEditLoading(true);
    try {
      await api.patch(`/api/v1/expenses/taxes/${editRecord.id}/`, data);
      setEditRecord(null);
      queryResult.refetch();
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Expense Taxes</h1><p className="mt-1 text-sm text-gray-600">Configure tax rules for expenses</p></div>
        <CreateButton permission="expense_taxes.create" label="Create Tax" onClick={() => setShowCreate(true)} />
      </div>
