...
const ENDPOINT = '/api/v1/recruitment/staffing-plans/';
const crud = createCrudHooks<Record<string, unknown>>({
  queryKey: 'staffing-plans',
...
const columns: ColumnDef<Record<string, unknown>>[] = [
  ...
  { key: 'filled', header: 'Filled', sortable: true },
...
const createFields: FieldConfig[] = [
  { key: 'department', label: 'Department', type: 'text', required: true, placeholder: 'Department name' },
  { key: 'designation', label: 'Designation', type: 'text', required: true, placeholder: 'Designation' },
  { key: 'vacancies', label: 'Vacancies', type: 'number', required: true, placeholder: 'Number of vacancies' },
  { key: 'fiscal_year', label: 'Fiscal Year', type: 'text', required: true, placeholder: 'e.g. 2024-2025' },
];
...
export default function Staffing() {
  ...
  const handleEdit = async (data: Record<string, unknown>) => {
    ...
    setEditLoading(true);
    try {
      await api.patch(`/api/v1/recruitment/staffing-plans/${editRecord.id}/`, data);
      setEditRecord(null);
      queryResult.refetch();
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Staffing Plans</h1><p className="mt-1 text-sm text-gray-600">Workforce planning, job requisitions, and headcount tracking</p></div>
        <CreateButton label="Create Plan" onClick={() => setShowCreate(true)} />
      </div>
