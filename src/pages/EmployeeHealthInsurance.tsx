...
const ENDPOINT = '/api/v1/employee-health-insurance/';
const crud = createCrudHooks<Record<string, unknown>>({
  queryKey: 'employee-health-insurance',
...
export default function EmployeeHealthInsurance() {
    ...
    ...columns,
    {
      ...
      render: (_value: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-1">
          <EditButton permission="health_insurance.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
          <DeleteButton permission="health_insurance.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
        </div>
      ),
    },
  ];
  ...
  const handleEdit = async (data: Record<string, unknown>) => {
    ...
    setEditLoading(true);
    try {
      await api.patch(`/api/v1/employee-health-insurance/${editRecord.id}/`, data);
      setEditRecord(null);
      queryResult.refetch();
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Health Insurance</h1><p className="mt-1 text-sm text-gray-600">Manage employee health insurance records</p></div>
        <CreateButton permission="health_insurance.create" label="Create Record" onClick={() => setShowCreate(true)} />
      </div>
      ...
      <CrudModal
        ...
        title="Create Health Insurance Record"
        fields={createFields}
