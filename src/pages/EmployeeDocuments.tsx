    ...
    ...columns,
    {
      ...
      render: (_value: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-1">
          <EditButton permission="documents.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
          <DeleteButton permission="documents.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <CreateButton permission="documents.create" label="Create Document" onClick={() => setShowCreate(true)} />
      </div>
