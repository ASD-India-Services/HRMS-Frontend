...
const ENDPOINT = '/api/v1/daily-work-summaries/';
const crud = createCrudHooks<Record<string, unknown>>({
  queryKey: 'daily-work-summaries',
...
const columns: ColumnDef<Record<string, unknown>>[] = [
  ...
  { key: 'tasks_completed', header: 'Tasks Completed', sortable: false },
  { key: 'hours_logged', header: 'Hours Logged', sortable: true },
];
...
const createFields: FieldConfig[] = [
  ...
  { key: 'tasks_completed', label: 'Tasks Completed', type: 'textarea', required: true, placeholder: 'Describe tasks completed today' },
  { key: 'hours_logged', label: 'Hours Logged', type: 'number', required: true, placeholder: 'Number of hours' },
];
...
export default function DailySummary() {
  ...
  const handleEdit = async (data: Record<string, unknown>) => {
    ...
    setEditLoading(true);
    try {
      await api.patch(`/api/v1/daily-work-summaries/${editRecord.id}/`, data);
      setEditRecord(null);
      queryResult.refetch();
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Daily Work Summary</h1><p className="mt-1 text-sm text-gray-600">Submit and review daily work summaries</p></div>
        <CreateButton label="Add Summary" onClick={() => setShowCreate(true)} />
      </div>
