...
const ENDPOINT = '/api/v1/grievance-types/';
const crud = createCrudHooks<Record<string, unknown>>({
  queryKey: 'grievance-types',
...
const createFields: FieldConfig[] = [
  { key: 'name', label: 'Name', type: 'text', required: true, placeholder: 'e.g. Workplace Safety' },
  { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe this grievance category' },
];

export default function GrievanceTypes() {
  ...
  const columnsWithActions: ColumnDef<Record<string, unknown>>[] = [
    ...
    { key: 'id', header: 'Actions', sortable: false, render: (_v: unknown, row: Record<string, unknown>) => (
      <div className="flex items-center gap-1">
        <EditButton permission="grievance_types.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
        <DeleteButton permission="grievance_types.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
      </div>
    )},
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Grievance Types</h1><p className="mt-1 text-sm text-gray-600">Manage grievance categories</p></div>
        <CreateButton permission="grievance_types.create" label="Create Type" onClick={() => setShowCreate(true)} />
      </div>
      ...
      <CrudModal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Grievance Type" fields={createFields} onSubmit={handleCreate} isLoading={createMutation.isPending} />
      <CrudModal isOpen={!!editRecord} onClose={() => setEditRecord(null)} title="Edit Grievance Type" fields={createFields} initialValues={editRecord ?? undefined} onSubmit={handleEdit} isLoading={editLoading} />
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => { if (deleteId) deleteMutation.mutate(deleteId, { onSuccess: () => setDeleteId(null) }); }} title="Delete Grievance Type" message="Delete this grievance type?" confirmLabel="Delete" variant="destructive" isLoading={deleteMutation.isPending} />
    </div>
  );
}
