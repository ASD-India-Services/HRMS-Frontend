    ...
    ...columns,
    {
      ...
      render: (_value: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-1">
          <EditButton permission="training_results.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
          <DeleteButton permission="training_results.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <CreateButton permission="training_results.create" label="Create Result" onClick={() => setShowCreate(true)} />
      </div>
