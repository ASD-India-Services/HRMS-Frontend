...
const ENDPOINT = '/api/v1/appraisals/templates/';
...
const createFields: FieldConfig[] = [
  { key: 'name', label: 'Template Name', type: 'text', required: true, placeholder: 'Enter template name' },
  { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe this template' },
...
export default function AppraisalTemplates() {
    ...
    ...columns,
    {
      ...
      render: (_value: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-1">
          <EditButton permission="appraisal_templates.edit" label="Edit" size="sm" onClick={() => setEditRecord(row)} />
          <DeleteButton permission="appraisal_templates.delete" label="Delete" size="sm" onClick={() => setDeleteId(row.id as string)} />
        </div>
      ),
    },
  ];
  ...
  const handleEdit = async (data: Record<string, unknown>) => {
    ...
    setEditLoading(true);
    try {
      await api.patch(`/api/v1/appraisals/templates/${editRecord.id}/`, data);
      setEditRecord(null);
      queryResult.refetch();
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Appraisal Templates</h1><p className="mt-1 text-sm text-gray-600">Manage appraisal evaluation templates</p></div>
        <CreateButton permission="appraisal_templates.create" label="Create Template" onClick={() => setShowCreate(true)} />
      </div>
